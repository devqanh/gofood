  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#F8F9FB] flex flex-col relative shadow-2xl overflow-x-hidden">
      ...
      <main className="px-5 flex-1 space-y-6 pt-4 pb-32">
        ...
        {/* User Info Section */}
        <section className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-50 space-y-6">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <h3 className="text-slate-900 font-extrabold text-lg tracking-tight">{MOCK_USER.name}</h3>
              <div className="flex items-center gap-2">
                <div className="px-2 py-0.5 bg-red-50 rounded-md border border-red-100">
                  <span className="text-red-600 text-[8px] font-black uppercase tracking-wider">HẠNG THÀNH VIÊN</span>
                </div>
                <span className="text-slate-400 text-[9px] font-bold tracking-widest uppercase">ID: 163562</span>
              </div>
            </div>
            <button 
              onClick={() => setShowOrderHistory(true)}
              className="flex items-center gap-1.5 text-[9px] font-bold text-slate-500 bg-slate-50 border border-slate-100 px-3 py-2 rounded-xl"
            >
          ...
          <div className="grid grid-cols-2 gap-4 border-y border-slate-50 py-5">
            <div className="space-y-1">
              <p className="text-slate-400 text-[8px] font-black uppercase tracking-widest">Tích lũy</p>
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-black text-slate-900 leading-none">{MOCK_USER.points.toLocaleString('vi-VN')}</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase">điểm</span>
              </div>
            </div>
            <div className="space-y-1">
              <p className="text-slate-400 text-[8px] font-black uppercase tracking-widest">Chi tiêu</p>
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-black text-slate-900 leading-none">{(MOCK_USER.spending / 1000000).toFixed(1)}M</span>
                <span className="text-[9px] font-bold text-slate-400 uppercase">VND</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              <span className="text-red-600 text-[9px] font-black uppercase tracking-widest">Mục tiêu {MOCK_USER.nextTier}</span>
              <span className="text-red-600 font-black text-[10px] bg-red-50 px-2 py-0.5 rounded-md">{progressPercentage.toFixed(1)}%</span>
            </div>
